...
'''
   유틸리티 함수 - axis 이용하여 행별 또는 열별로 연산
    ...
    3. np.argmax : 1, 2차원 최대값에 해당하는 인덱스 반환 (중요)
    4. np.argmin : 1, 2차원 최소값에 해당하는 인덱스 반환 (중요)
...
[[ 6  2  4]
 ...
 [66  3 21]]
'''
print("3. 2차원 배열, 각 열에서 최대 index값:", np.argmax(data, axis=0))  # 위 + 아래  [2 2 1]
print("4. 2차원 배열, 각 열에서 최소 index값:", np.argmin(data, axis=0))  # 위 + 아래  [0 1 0]
print("5. 2차원 배열, 각 행에서 최대 index값:", np.argmax(data, axis=1))  # 왼쪽 + 오른쪽 [0 2 0]
...
#  [100  67  33]
#  [ 99  44  77]]
data4 = np.sort(arr, axis=0)  # 위+아래 컬럼단위로 정렬
print(data4)
# [[  8   2   4]
#  [ 99  44  33]
#  [100  67  77]]
data4 = np.sort(arr, axis=1)  # 왼쪽+오른쪽 행단위로 정렬
print(data4)
# [[  2   4   8]
#  [ 33  67 100]
#  [ 44  77  99]]
...
# 8. 행렬곱( dot product(점곱), 곱해서 합 ) ==>  np.dot(a,b),  a @ b  ( 매우 중요하다 )
'''
