'''
    1. outer 병합
      ...
      가. 공통 컬럼 이용
          new_df = pd.merge(df, df2,  how=“left|right|outer”  on=“공통컬럼명”)
          new_df = pd.merge(df, df2,  how=“left|right|outer”  on=[“공통컬럼명”,“공통컬럼명2”]) => 복합컬럼

          new_df = pd.merge(df, df2,  how=“left|right|outer”  on=“공통컬럼명”, indicator=True)
                  .query("조건식").drop(columns=[컬럼,.])



       나. 비공통 컬럼 이용
          new_df = pd.merge(df, df2, how='left|right|outer', left_on="컬럼명", right_on="컬럼명")
